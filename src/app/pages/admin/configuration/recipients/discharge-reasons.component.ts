...
@Component({
  selector: 'app-discharge-reasons',
  templateUrl: './discharge-reasons.component.html',
  styles: []
})
export class DischargeReasonsComponent implements OnInit {
  ...
  modalVariables:any;
  inputVariables:any;
  title:string = "Add Discharge Reasons"
  tocken: any;
  pdfTitle: string;
  tryDoctype: any;
  ...
  showAddModal() {
    this.title = "Add Discharge Reasons"
    this.resetModal();
    this.modalOpen = true;
  }
  ...
  showEditModal(index: any) {
    // debugger;
    this.title = "Edit Discharge Reasons"
    this.isUpdate = true;
    this.current = 0;
    this.modalOpen = true;
      const { 
  ...
  loadData(){
    this.loading = true;
    this.menuS.getDataDomainByType("REASONCESSSERVICE",this.check).subscribe(data => {
      this.tableData = data;
      this.loading = false;
    });
  }
            ...
            if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Discharge Reasons Changed',
                  actionOn:'REASONCESSSERVICE',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Discharge Reasons Deleted',
                actionOn:'REASONCESSSERVICE',
  ...
  generatePdf(){
    ...
    var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='REASONCESSSERVICE'";
    const data = {
      "template": { "_id": "0RYYxAkMCftBE9jc" },
      "options": {
        "reports": { "save": false },
        "txtTitle": "Recipient Discharge Reasons List",
        "sql": fQuery,

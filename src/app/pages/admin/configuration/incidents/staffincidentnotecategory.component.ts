...
@Component({
  ...
  templateUrl: './staffincidentnotecategory.component.html',
  styles: []
})
export class StaffincidentnotecategoryComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("STFIMNTECAT",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
    ...
    showEditModal(index: any) {
      this.title = "Edit Staff Incident Note Category"
      this.isUpdate = true;
      this.current = 0;
      this.modalOpen = true;
      const { 
    ...
    save() {
      ...
      if(!this.isUpdate){   
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Staff Incident Note Categories Changed',
                  actionOn:'STFIMNTECAT',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Staff Incident Note Categories Deleted',
                actionOn:'STFIMNTECAT',
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='STFIMNTECAT'";

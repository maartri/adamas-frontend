  ...
  constructor(
    ...
    save() {
      ...
      if(!this.isUpdate){         
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Recipients Group Changed',
                  actionOn:'RECIPTYPE',
        ...
        loadData(){
          this.loading = true;
          this.menuS.getDataDomainByType("RECIPTYPE",this.check).subscribe(data => {
            this.tableData = data;
            this.loading = false;
          });
        }
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Recipients Group Deleted',
                actionOn:'RECIPTYPE',
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='RECIPTYPE'";

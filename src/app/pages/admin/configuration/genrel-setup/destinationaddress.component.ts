...
export class DestinationaddressComponent implements OnInit {
  ...
  loadData(){
    let sql ="SELECT *,ROW_NUMBER() OVER(ORDER BY Name) AS row_num FROM HumanResourceTypes Where ISNULL(HumanResourceTypes.DeletedRecord, 0) = 0 AND [Group] like 'DESTINATION'";
    this.loading = true;
  ...
  generatePdf(){
    ...
    var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Name) AS Field1,[Name] as Field2,[Type] as Field3,[Address1] as Field4,[phone1] as Field5,[Fax] as Field6,[EndDate] as Field7 from HumanResourceTypes  Where ISNULL(HumanResourceTypes.DeletedRecord, 0) = 0 AND [Group] like 'DESTINATION'";

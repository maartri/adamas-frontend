...
export class LifecycleEventsComponent implements OnInit {
  ...
  temp_title: any;
   constructor(
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("LIFECYCLEEVENTS",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='LIFECYCLEEVENTS'";

...
export class StaffRemindersComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      this.loading = true;
          this.menuS.getDataDomainByType("STAFFALERT",this.check).subscribe(data => {
            this.tableData = data;
            this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Staff Reminders/Alerts Changed',
                  actionOn:'STAFFALERT',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Staff Reminders/Alerts Deleted',
                actionOn:'STAFFALERT',
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" AND Domain='STAFFALERT'";

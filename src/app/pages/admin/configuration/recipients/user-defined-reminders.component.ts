...
export class UserDefinedRemindersComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("RECIPIENTALERT",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){         
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Recipient User Defined Reminders/Reviews/Alerts Changed',
                  actionOn:'RECIPIENTALERT',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'User Defined Reminders/Reviews/Alerts Deleted',
                actionOn:'RECIPIENTALERT',
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='RECIPIENTALERT'";
          
          const data = {
            "template": { "_id": "0RYYxAkMCftBE9jc" },
            "options": {
              "reports": { "save": false },
              "txtTitle": "Recipient User Defined Reminders/Alerts List",
              "sql": fQuery,

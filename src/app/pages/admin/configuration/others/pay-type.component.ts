...
export class PayTypeComponent implements OnInit {
  ...
  whereString :string="WHERE processClassification <> 'INPUT' AND ISNULL(DeletedRecord, 0) = 0 AND (enddate Is Null OR enddate >= GETDATE())";
  inputForm: FormGroup;
  modalVariables:any;
  inputVariables:any;
  timesteps:Array<any>;
  ...
  constructor(
    ...
    loadData(){
      ...
      this.loading = true;
      this.menuS.GetlistagencyPayTypes(this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
        this.cd.detectChanges();
      });
    ...
    activatePaytype(data: any) {
      this.postLoading = true;     
      const group = this.inputForm;
      this.menuS.activatePayTypeslist(data.recordNumber)
      .pipe(takeUntil(this.unsubscribe)).subscribe(data => {
        if (data) {
          this.globalS.sToast('Success', 'Data Reactive!');
          this.loadData();
          return;
        }
      });
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY [Title]) AS Field1,[title] AS [Field2], [rostergroup] AS [Field3], [minorgroup] AS [Field4], [amount] AS [Field5], [unit] AS [Field6],[accountingidentifier] AS [Field7],[paygroup] AS [Field8] FROM itemtypes "+this.whereString+" ORDER BY title";

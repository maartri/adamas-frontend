...
export class RecipientPrefrencesComponent implements OnInit {
  ...
  temp_title: any;
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("RECIPPREF",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Recipient Preferences Changed',
                  actionOn:'RECIPPREF',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Recipient Prefrences Deleted',
                actionOn:'RECIPPREF',
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='RECIPPREF'";

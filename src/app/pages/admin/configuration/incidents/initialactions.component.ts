...
export class InitialactionsComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("IMActionInit",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){     
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY recordNumber) AS Field1,Description as Field2,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='IMActionInit'";

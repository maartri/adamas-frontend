...
@Component({
  selector: 'app-staff-notes-categories',
  templateUrl: './staff-notes-categories.component.html',
  styles: []
})
export class StaffNotesCategoriesComponent implements OnInit {
  ...
  title:string = "Add HR Staff Notes Categories"
  tocken: any;
  pdfTitle: string;
  tryDoctype: any;
  ...
  constructor(
    ...
    showAddModal() {
      this.title = "Add HR Staff Notes Categories"
      this.resetModal();
      this.modalOpen = true;
    }
    ...
    showEditModal(index: any) {
      // debugger;
      this.title = "Edit HR Staff Notes Categories"
      this.isUpdate = true;
      this.current = 0;
      this.modalOpen = true;
      const { 
    ...
    loadData(){
      this.loading = true;
          this.menuS.getDataDomainByType("HRGROUPS",this.check).subscribe(data => {
            this.tableData = data;
            this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){         
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'HR Staff Notes Categories Changed',
                  actionOn:'HRGROUPS',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'HR Staff Notes Categories Deleted',
                actionOn:'HRGROUPS',
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS row_num,Description as Field2,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='HRGROUPS'";
          
          const data = {
            "template": { "_id": "0RYYxAkMCftBE9jc" },
            "options": {
              "reports": { "save": false },
              "txtTitle": "Hr Notes Categories List",
              "sql": fQuery,

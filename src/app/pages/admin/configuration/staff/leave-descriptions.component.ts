...
export class LeaveDescriptionsComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      let sql ="SELECT ROW_NUMBER() OVER(ORDER BY Description) AS row_num, Description as name,recordNumber from DataDomains Where ISNULL(DataDomains.DeletedRecord, 0) = 0 AND Domain='LEAVEAPP' ";
      this.loading = true;

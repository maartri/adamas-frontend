...
export class CarePlanTypesComponent implements OnInit {
  ...
  temp_title: any;
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("CAREPLANTYPES",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Care Plan Types Changed',
                  actionOn:'CAREPLANTYPES',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Plan Type Deleted',
                actionOn:'CAREPLANTYPES',
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='CAREPLANTYPES'";

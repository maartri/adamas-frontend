...
export class FundingRegionsComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      // let sql ="SELECT ROW_NUMBER() OVER(ORDER BY Description) AS row_num,Description as name,recordNumber,DeletedRecord as is_deleted from DataDomains "+this.whereString+" Domain='FUNDREGION'";
      // this.loading = true;
      ...
      // });
      this.loading = true;
      this.menuS.getDataDomainByType("FUNDREGION",false).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){         
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 from DataDomains "+this.whereString+" Domain='FUNDREGION' AND ISNULL(DataDomains.DeletedRecord, 0) = 0";

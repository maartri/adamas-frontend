...
export class ConsentTypesComponent implements OnInit {
  ...
  tableData: Array<any>;
    ...
    modalVariables:any;
    inputVariables:any;
    title:string = "Add Consent Types"
    tocken: any;
    pdfTitle: string;
    tryDoctype: any;
...
this.buildForm();
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("RECIPIENTCONSENTS",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Recipient Consents Changed',
                  actionOn:'RECIPIENTCONSENTS',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Consent Types Deleted',
                actionOn:'RECIPIENTCONSENTS',
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='RECIPIENTCONSENTS'";
      
      const data = {
        "template": { "_id": "0RYYxAkMCftBE9jc" },
        "options": {
          "reports": { "save": false },
          "txtTitle": "Recipient Consent Types List",
          "sql": fQuery,

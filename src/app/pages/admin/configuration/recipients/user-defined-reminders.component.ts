...
export class UserDefinedRemindersComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      let sql ="SELECT ROW_NUMBER() OVER(ORDER BY Description) AS row_num, Description as name,recordNumber from DataDomains where Domain='RECIPIENTALERT' ";
      this.loading = true;
    ...
    save() {
      ...
      generatePdf(){
        ...
        var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 from DataDomains where Domain='RECIPIENTALERT'";

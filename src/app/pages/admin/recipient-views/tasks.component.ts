...
export class TasksComponent implements OnInit {
  ...
  whereString :string="WHERE ISNULL(DataDomains.DeletedRecord) AND (EndDate Is Null OR EndDate >= GETDATE() AND ";
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("Task",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 from DataDomains "+this.whereString+" Domain='Task'";

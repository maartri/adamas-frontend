...
@Component({
  selector: 'app-items-consumables',
...
export class ItemsConsumablesComponent implements OnInit {
  ...
  constructor(
    ...
    showEditModal(index: any) {
      this.title = "Edit Items/Consumables"
      this.isUpdate = true;
      this.current = 0;
      this.modalOpen = true;
    ...
    save() {
      this.loading = true;
      if(!this.isUpdate){
        this.menuS.postItemConsumables(this.inputForm.value)
        .subscribe(data => {
        ...
        this.menuS.updateItemConsumables(this.inputForm.value)
        .subscribe(data => {
      ...
      if(this.addOrEdit == 0){
        ...
        generatePdf(){
          ...
          const data = {
            ...
            "options": {
              "reports": { "save": false },
              "txtTitle": "Item Consumables List",
              "sql": fQuery,

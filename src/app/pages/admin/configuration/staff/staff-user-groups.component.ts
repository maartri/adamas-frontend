...
export class StaffUserGroupsComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      this.loading = true;
          this.menuS.getDataDomainByType("STAFFTYPE",this.check).subscribe(data => {
            this.tableData = data;
            this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){      
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'User Group Changed',
                  actionOn:'STAFFTYPE',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'User Groups Deleted',
                actionOn:'STAFFTYPE',
        ...
        generatePdf(){
          ...
          this.loading = true;
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY recordNumber) AS Field1,Description as Field2,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='STAFFTYPE'";

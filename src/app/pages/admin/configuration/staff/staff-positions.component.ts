...
export class StaffPositionsComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      this.loading = true;
          this.menuS.getDataDomainByType("STAFFPOSITION",this.check).subscribe(data => {
            this.tableData = data;
            this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){      
            ...
            if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Staff Positions Changed',
                  actionOn:'STAFFPOSITION',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Staff Positions Deleted',
                actionOn:'STAFFPOSITION',
        ...
        generatePdf(){
          ...
          this.loading = true;
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY recordNumber) AS Field1,Description as Field2,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='STAFFPOSITION'";

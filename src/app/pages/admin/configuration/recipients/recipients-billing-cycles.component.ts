  ...
  constructor(
    ...
    save() {
      ...
      if(!this.isUpdate){         
        ...
        loadData(){
          let sql ="SELECT ROW_NUMBER() OVER(ORDER BY Description) AS row_num, Description as name,recordNumber from DataDomains Where ISNULL(DataDomains.DeletedRecord, 0) = 0 AND Domain='BILLINGCYCLE'";
          this.loading = true;
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 from DataDomains Where ISNULL(DataDomains.DeletedRecord, 0) = 0 AND Domain='BILLINGCYCLE'";

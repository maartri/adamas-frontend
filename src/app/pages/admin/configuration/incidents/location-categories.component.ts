...
export class LocationCategoriesComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      let sql ="select ROW_NUMBER() OVER(ORDER BY recordNumber) AS row_num,Description as name,recordNumber from DataDomains where Domain='IMLocation' ";
      this.loading = true;
    ...
    save() {
      ...
      if(!this.isUpdate){         
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY recordNumber) AS Field1,Description as Field2 from DataDomains where Domain='IMLocation'";

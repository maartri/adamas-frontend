...
this.buildForm();
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("RECIPOPNOTEGROUPS",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'OP Notes Groups Changed',
                  actionOn:'RECIPOPNOTEGROUPS',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'OP Notes Groups Deleted',
                actionOn:'RECIPOPNOTEGROUPS',
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='RECIPOPNOTEGROUPS'";
      
      const data = {
        "template": { "_id": "0RYYxAkMCftBE9jc" },
        "options": {
          "reports": { "save": false },
          "txtTitle": "Recipient OP Notes Categories List",
          "sql": fQuery,

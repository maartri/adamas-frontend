...
export class ServiceDeciplinesComponent implements OnInit {
  ...
  constructor(
    ...
    loadData(){
      this.loading = true;
          this.menuS.getDataDomainByType("DISCIPLINE",this.check).subscribe(data => {
            this.tableData = data;
            this.loading = false;
      });
    }
    ...
    save() {
      ...
      if(!this.isUpdate){      
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Service Disciplines Changed',
                  actionOn:'DISCIPLINE',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Service Disciplines Deleted',
                actionOn:'DISCIPLINE',
        ...
        generatePdf(){
          ...
          var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='DISCIPLINE'";

...
export class CaseNoteCategoriesComponent implements OnInit {
  ...
  temp_title: any;
    ...
    loadData(){
      this.loading = true;
      this.menuS.getDataDomainByType("CASENOTEGROUPS",this.check).subscribe(data => {
        this.tableData = data;
        this.loading = false;
      });
    }
            ...
            this.inputVariables = {
              ...
              if (data)
              {
                this.timeS.postaudithistory({
                  ...
                  auditDescription:'Case Notes Groups Changed',
                  actionOn:'CASENOTEGROUPS',
          ...
          .pipe(takeUntil(this.unsubscribe)).subscribe(datas => {
            if (datas) {
              this.timeS.postaudithistory({
                Operator:this.tocken.user,
                actionDate:this.globalS.getCurrentDateTime(),
                auditDescription:'Case Notes Group Deleted',
                actionOn:'CASENOTEGROUPS',
    ...
    generatePdf(){
      ...
      var fQuery = "SELECT ROW_NUMBER() OVER(ORDER BY Description) AS Field1,Description as Field2 ,CONVERT(varchar, [enddate],105) as Field3 from DataDomains "+this.whereString+" Domain='CASENOTEGROUPS'";
      
      const data = {
        "template": { "_id": "0RYYxAkMCftBE9jc" },
        "options": {
          "reports": { "save": false },
          "txtTitle": "Recipient Case Notes Categories List",
          "sql": fQuery,
